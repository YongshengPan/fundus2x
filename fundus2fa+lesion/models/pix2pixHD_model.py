...
class Pix2PixHDModel(BaseModel):
    ...
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss,seg):
        flags = (True, use_gan_feat_loss, use_vgg_loss, True, True,seg)
        def loss_filter(g_gan, g_gan_feat, g_vgg, d_real, d_fake,g_seg):
            return [l for (l,f) in zip((g_gan,g_gan_feat,g_vgg,d_real,d_fake,g_seg),flags) if f]
        return loss_filter
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.loss_filter = self.init_loss_filter(not opt.no_ganFeat_loss, not opt.no_vgg_loss,self.seg_loss)
            ...
            self.loss_names = self.loss_filter('G_GAN','G_GAN_Feat','G_VGG','D_real', 'D_fake','G_seg')
    ...
    def forward(self, label, inst, image, feat, infer=False):
        ...
        if self.use_features:
            ...
            # print(input_label.shape,feat_map.shape)        
        else:
        ...
        return [ self.loss_filter( loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_D_real, loss_D_fake, loss_G_seg ), None if not infer else fake_image ]
